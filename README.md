...
`safari close-window` closes Safari's front window by default. Use `--window-id <id>` to close a specific window by stable Safari window identifier. Identifier-targeted close first resolves and focuses the stable id in its owning Safari process, captures the focused window only from that process, and verifies that the captured window leaves the process's Accessibility inventory. The structural close-button fallback runs only while cross-process readback still contains the target, so an already removed target cannot cause another focused window to close. Final readback excludes Safari's zero-tab scripting ghosts as well as absent windows. An already absent stable id is treated as successfully closed without focusing another window.
