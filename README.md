...
- Saved tab-group create/rename/delete is driven by accessibility:
  ...
  - rename opens a brand-new profile window, retries selection until the exact sidebar row and cell report selected, invokes `RenameTabGroupMenuItem` only inside `SafariContextMenu`, confirms the inline edit field, and verifies that the same identifier has the requested name
