...
- `SafariFileMenu.createTabGroupFromCurrentTabs(processIdentifier:)` opens the File menu only in the requested owning process and identifies its item by the stable accessibility identifier `NewTabGroupWithTabsMenuItem` instead of a localized title. When a newly opened window exposes `AXEnabled=false`, each bounded retry dismisses and reopens the menu so Safari rebuilds the command state before another structural lookup. The command issues `AXPress` only after the item becomes enabled; a persistently disabled item reports a dedicated error without being pressed, and every unsuccessful menu is dismissed structurally.
