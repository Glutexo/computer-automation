...
- Standalone rename retries the exact sidebar row/cell selection until the target row reports selected, then uses the outline's structural `AXShowMenu` action and accepts only `RenameTabGroupMenuItem` beneath the exact `SafariContextMenu`; title-element menu actions, unverified selection, and application-wide menu-item matches are not accepted.
